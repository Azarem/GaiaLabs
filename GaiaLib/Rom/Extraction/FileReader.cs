...
public class FileReader(byte[] romData, DbRoot dbRoot)
{
    ...
    public async Task Extract(string outPath)
    {
        ...
        foreach (var file in _dbRoot.Files)
        {
            ...
            if (file.Compressed == true)
            {
                ...
                await CopyBytes(fileStream, data, 0, data.Length, header, file.Type == BinType.Palette);
            }
            else
            {
    ...
    )
    {
        ...
        await outStream.WriteAsync(data, position, length);
